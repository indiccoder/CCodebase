...
int fibonacci(int limit);   // Function Prototype that takes int arguments and returns an int.
...
int fibonacci(int limit)    // Fibonacci Function starts and stores the value of 'lim' into 'limit'.
{
...
return 0;   // Returns 0 as the output is achieved in the function itself.
}